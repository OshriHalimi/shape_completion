...
for root, dirnames, filenames in os.walk(read_path):
    for filename in fnmatch.filter(filenames, '*.obj*'):
        if flag == False:
            if os.path.join(root, filename) == '/run/user/1000/gvfs/smb-share:server=132.68.36.59,share=data/ShapeCompletion/Mixamo/Blender/MPI-FAUST/050/Catwalk Walk/041.obj':
                flag = True
