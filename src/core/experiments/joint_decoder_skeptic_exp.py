...
def parser():
    ...
    p.add_argument('--exp_name', type=str, default='joint_decoder_skeptic_exp', help='The experiment name. Leave empty for default')
    ...
    p.add_argument('--loss_class', type=str, choices=['BasicLoss', 'SkepticLoss'], default='JointDecoderSkepticLoss',
                   help='The loss class')
...
def train_main():
    banner('Network Init')
    nn = F2PEncoderJointDecoderSkeptic(parser())
    nn.identify_system()
