...
def train_main():
    ...
    tr_ldr = tr_ds.loaders(s_nums=hp.counts[0], s_transform=[Center(keys=('gt','tp1','tp2'))], batch_size=hp.batch_size, device=hp.dev,
                           n_channels=hp.in_channels, method='rand_ff2p',s_dynamic=True)
    ...
    val_ldr = val_ds.loaders(s_nums=hp.counts[1], s_transform=[Center(keys=('gt','tp1','tp2'))], batch_size=hp.batch_size, device=hp.dev,
                             n_channels=hp.in_channels, method='rand_ff2p') # Should have been f2p, but it is too big
    ...
    ts_ldr = ts_ds.loaders(s_nums=hp.counts[2], s_transform=[Center(keys=('gt','tp1','tp2'))], batch_size=hp.batch_size, device=hp.dev,
                           n_channels=hp.in_channels, method='rand_ff2p')
