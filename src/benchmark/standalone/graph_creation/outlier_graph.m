...
load('outlier_cache.mat','stats');
fnames = fieldnames(stats);

N = length(fnames); % Assuming noise experiments are the only fields
X = zeros(N,1);
...
for i=1:N
    ...
    assert(length(noise_ids)==2);
    noise_id = noise_ids{1};
...
subplot(3,1,1); 
plot_graph(X,me_means,me_stds);title('\textbf{Euclidean Distance Vs Number of Outliers}','Interpreter','Latex','FontSize',15); 
% print(gcf,'noise1.png','-dpng','-r1200');
subplot(3,1,2); 
plot_graph(X,gt2res_means,gt2res_stds); title('\textbf{GT to Reconstruction Vs Number of Outliers}','Interpreter','Latex','FontSize',15); 
% print(gcf,'noise2.png','-dpng','-r1200');
subplot(3,1,3); 
plot_graph(X,res2gt_means,res2gt_stds); title('\textbf{Reconstruction to GT Vs Number of Outliers}','Interpreter','Latex','FontSize',15); 
% print(gcf,'noise_analysis.png','-dpng','-r1200');

function plot_graph(X,means,stds)

errorbar(X,means,stds,'-s','LineWidth',2,'MarkerSize',6,'MarkerEdgeColor',[0.3350 0.3780 0.6840],'MarkerFaceColor',[0.6350 0.0780 0.1840],'color',[0.4660 0.6740 0.1880]); 
...
xlabel('\textbf{Number of Outliers}','Interpreter','Latex','FontSize',14);
