...
for i=1:N
    ...
    noise_id = noise_ids{2};
    noise_id = ['0.' , noise_id(1:end-1)]; % Remove the 2
...
subplot(3,1,1); 
plot_graph(X,me_means,me_stds);title('\textbf{Euclidean Distance Vs Noise STD}','Interpreter','Latex','FontSize',15); 
% print(gcf,'noise1.png','-dpng','-r1200');
subplot(3,1,2); 
plot_graph(X,gt2res_means,gt2res_stds); title('\textbf{GT to Reconstruction Vs Noise STD}','Interpreter','Latex','FontSize',15); 
% print(gcf,'noise2.png','-dpng','-r1200');
subplot(3,1,3); 
plot_graph(X,res2gt_means,res2gt_stds); title('\textbf{Reconstruction to GT Vs Noise STD}','Interpreter','Latex','FontSize',15); 
...
xlabel('\textbf{Noise STD [cm]}','Interpreter','Latex','FontSize',14);
