function [rotmat,cornerpoints,h,volume,surface,edgelength] = minboundbox(x,y,z,metric,level)
...
elseif ~ischar(metric)
  ...
  % check for 'v', 's' or 'e'
  metric = lower(metric(:)');
  ind = validatestring(metric,{'volume','surface','edges'});
  if isempty(ind)
...
try
 ...
 Ki = unique(K(:));
 [~,loc] = ismember(K(:),Ki);K = reshape(loc,size(K));
 x = x(Ki);
 y = y(Ki);
 z = z(Ki);
%  n1 =length(x);
