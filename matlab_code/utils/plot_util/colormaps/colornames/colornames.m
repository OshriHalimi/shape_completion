...
%   'CIEDE2000', 'CIE94' (default), 'CIE76', 'DIN99', 'CMCl:c', or 'RGB'.
...
%  deltaE = CharRowVector: 'CIEDE2000', 'CIE94'*, 'CIE76', 'DIN99', 'CMCl:c', or 'RGB'.
%
%%% Outputs:
...
%
fnc = fieldnames(data);
dtE = {'CIEDE2000','CIE94','CIE76','DIN99','CMCl:c','RGB'};
%
...
if nargin==0
	if nargout
		...
		out(4,:) = cellfun(@(p)data.(p).source,fnc,'Uni',0);
		nml = [max(cellfun('length',out(1,:)),[],2),...
			numel(sprintf('%d',max([out{2,:}])))];
		fmt = sprintf('%%%ds  %%%dd  %%-s  Source: %%-s\\n',nml);
		fprintf(1,fmt,out{:}) %#ok<PRTCAL>
	end
	return
end
%
...
error(['The palette %1$s does not contain these colors:%2$s\n\n%3$s\n'...
	...
	'or COLORNAMES_CUBE(%1$s) to view the palette in a 3D cube.'],pnm,txt,str) %#ok<SPERR>
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%cnNoMatch
