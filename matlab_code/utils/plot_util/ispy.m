...
% set(fh, 'color', 'w')
cameratoolbar;
xlabel(sprintf('pos = %d : neg = %d : inf = %d : nan = %d',n_pos,n_neg,n_infinite,n_nan)); 
...
%     np = sum(pinds); % num of pos elems
    ...
    % Override: Did not like the different sizes:
    %Sp(:) = meanval;
    % Plot
    cla; hold on;
    scatter(Yp(1:nn),    Xp(1:nn),    Sp(1:nn),    'r','filled'); % neg elems
    scatter(Yp(nn+1:end),Xp(nn+1:end),Sp(nn+1:end),'b','filled'); % pos elems
    % NaN and Inf
    if ~isempty(Xnan)
